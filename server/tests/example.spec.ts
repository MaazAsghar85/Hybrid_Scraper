...
async function launchBrowser() {
    browser[browsers] = await playwright.chromium.launch({
        slowMo: 500,
        headless: true,  // Change to true if you want headless mode
        args: ['--window-position=0,0'],
    });
    context[browsers] = await browser[activeBrowser].newContext({ viewport: { width: 1050, height: 760 } });
        ...
        if (openPost) {
            ...
            while (true) {
                try {
                    ...
                    const hrefs = await page[activeWindow].$$eval('ul._a9z6._a9za a', (links: any[]) =>
                        links
                    ...
                    hrefs.forEach((href: string) => collectedUrls.add(href));
...
async function DownloadPostMedia(username: string, platform: string) {
    try {
        ...
        if (post) {
            const postImages = await page[activeWindow].$$eval(postImagesSelector, (imgs: any[]) =>
                imgs.map((img) => (img as HTMLImageElement).src).filter(src => src !== undefined && src !== '')
            );
            postImages.forEach(async (url: string | https.RequestOptions | URL, index: number) => {
...
async function DownloadPostMediaAll(username: string, platform: string) {
    ...
    try {
        ...
        while (noChangeCount < maxNoChangeCount) {
            ...
            const postImages = await page[activeWindow].$$eval(postImagesSelector, (imgs: any[]) =>
                imgs.map((img) => (img as HTMLImageElement).src).filter((src: string | undefined) => src !== undefined && src !== '')
            );
...
async function DownloadFollow(username: string, platform: string, follow: string, maxScrolls: string) {
    ...
    try {
        ...
        while (scrolls < max / divisor && attempts < 10) {
            const currentHeight = await page[activeWindow].evaluate((selector: any) => {
...
const server = net.createServer((socket) => {
    ...
    socket.on('data', async(data) => {
        ...
        if (parts[0]=='goto'){
            ...
            const screenshotPath = path.join(screenshotDir, parts[1]);
            ...
            try {
                await page[activeWindow].screenshot({ path: screenshotPath });
                console.log(`Screenshot saved at: ${screenshotPath}`);
            } catch (error) {
                console.log("Error: Unable to save screenshot, maybe wrong file extension!");
            }
        }
            ...
            try {
                ...
                fs.writeFileSync(`server/${parts[1]}`, JSON.stringify(cookies, null, 2));
            } catch {
            ...
            try {
                islogin = true;
                const fs = require('fs');
                const cookies = JSON.parse(fs.readFileSync(`server/${parts[1]}`, 'utf8'));
